...
def input_form_view(request):
    ...
    return render(request, 'm8/index.html', context)
...
def output_view(request):
    ...
    if request.method == 'POST':
        ...
        m8_runs = request.session.get('m8_runs', [])

        new_m8_runs = {
            'run_id': len(m8_runs) + 1,  
            'title': 'M8 Run ' + str(len(m8_runs) + 1),
            'form_data': form_data,
            'output': model_output
        }

        m8_runs.append(new_m8_runs)
        request.session['m8_runs'] = m8_runs
        ...
        return render(request, 'm8/run_output.html', context)
    

@login_required(login_url='/login/')
def fetch_m8_error_view(request):
    ...
    error_html = render_to_string('m8/error.html', error_context, request=request)
    return HttpResponse(error_html)


@login_required(login_url='/login/')
def current_m8_result_view(request):
    m8_runs = request.session.get('m8_runs', [])

    # Get the last run if available, or None if mvoh_runs is empty
    last_run = m8_runs[-1] if m8_runs else None
...
def run_m8_model_view(request):
    if request.method == 'POST':
        ...
        return render(request, 'm8/summary_result.html', context)
