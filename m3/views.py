...
def input_form_view(request):
    ...
    return render(request, 'm3/index.html', context)
...
def output_view(request):
    ...
    if request.method == 'POST':
        ...
        new_m3_runs = {
            'run_id': len(m3_runs) + 1,  
            'title': 'M3 Run ' + str(len(m3_runs) + 1),
            'form_data': form_data,
            'output': model_output
        }

        m3_runs.append(new_m3_runs)
        request.session['m3_runs'] = m3_runs
        ...
        return render(request, 'm3/run_output.html', context)
    

@login_required(login_url='/login/')
def fetch_m3_error_view(request):
    ...
    error_html = render_to_string('m3/error.html', error_context, request=request)
    return HttpResponse(error_html)


@login_required(login_url='/login/')
def current_m3_result_view(request):
    m3_runs = request.session.get('m3_runs', [])

    # Get the last run if available, or None if mvoh_runs is empty
    last_run = m3_runs[-1] if m3_runs else None
...
def run_m3_model_view(request):
    if request.method == 'POST':
        ...
        return render(request, 'm3/summary_result.html', context)
