...
from .model_result_m4 import model_output_data

from runs.models import Run


# M4 form processing function
def process_m4_form(rq):
    # Fetch form data
...
def input_form_view(request):
    ...
    return render(request, 'm4/index.html', context)
...
def output_view(request):
    if request.method == 'POST':
        form_data = process_m4_form(request)
        ...
        new_m4_runs = {
            'run_id': len(m4_runs) + 1,  
            'title': 'M4 Run ' + str(len(m4_runs) + 1),
            'form_data': form_data,
            'output': model_output_data
        }
        m4_runs.append(new_m4_runs)
        request.session['m4_runs'] = m4_runs
        ...
        return render(request, 'm4/run_output.html', context)
    

@login_required(login_url='/login/')
def fetch_m4_error_view(request):
    ...
    error_html = render_to_string('m4/error.html', error_context, request=request)
    return HttpResponse(error_html)


@login_required(login_url='/login/')
def current_m4_result_view(request):
    m4_runs = request.session.get('m4_runs', [])

    # Get the last run if available, or None if mvoh_runs is empty
    last_run = m4_runs[-1] if m4_runs else None
...
def run_m4_model_view(request):
    if request.method == 'POST':
        ...
        return render(request, 'm4/summary_result.html', context)
