...
# M6 form processing function
def process_m6_form(rq):
    # Fetch form data
...
def input_form_view(request):
    ...
    return render(request, 'm6/index.html', context)
...
def output_view(request):
    if request.method == 'POST':
        form_data = process_m6_form(request)
        ...
        m6_runs = request.session.get('m6_runs', [])
        new_m6_runs = {
            'run_id': len(m6_runs) + 1,  
            'title': 'M6 Run ' + str(len(m6_runs) + 1),
            'form_data': form_data,
            'output': model_output_data
        }
        m6_runs.append(new_m6_runs)
        request.session['m6_runs'] = m6_runs
        ...
        return render(request, 'm6/run_output.html', context)
    

@login_required(login_url='/login/')
def fetch_m6_error_view(request):
    ...
    error_html = render_to_string('m6/error.html', error_context, request=request)
    return HttpResponse(error_html)


@login_required(login_url='/login/')
def current_m6_result_view(request):
    m6_runs = request.session.get('m6_runs', [])

    # Get the last run if available, or None if mvoh_runs is empty
    last_run = m6_runs[-1] if m6_runs else None
...
def run_m6_model_view(request):
    if request.method == 'POST':
        ...
        return render(request, 'm6/summary_result.html', context)
